...
-- Stage changes, written by trigger rather than by the application: the board
-- moves deals by drag & drop straight through the data provider, so an
-- application-side write would miss every one of them (NOS-819, NOS-958 §7).
...
alter table public.deal_stage_history
    add constraint deal_stage_history_changed_by_fkey foreign key (changed_by) references public.sales(id) on delete set null;
...
-- The side panel reads "most recent first, for this deal".
create index deal_stage_history_deal_idx on public.deal_stage_history using btree (deal_id, changed_at desc);
