...
/**
 * Resource lifecycle callbacks shared by the demo provider and the browser-test
 * harness (src/test/createTestDataProvider.ts), so tests exercise the same
 * side effects as the app — notably the contact task-count bookkeeping.
 */
export const crmLifecycleCallbacks = [
  {
  ...
  } satisfies ResourceCallbacks<Sale>,
  {
    ...
    beforeCreate: async (createParams, dataProvider) => {
      const params = {
        ...
        data: {
          ...createParams.data,
          first_seen: createParams.data.first_seen ?? new Date().toISOString(),
  ...
  } satisfies ResourceCallbacks<Contact>,
  {
    ...
    afterCreate: async (result, dataProvider) => {
      ...
      // (`tasks_owner_check` only requires one of the two), and `nb_tasks`
      // counts a *contact's* tasks — there is nothing to bump. Asking for
      // contact `undefined` used to fail the whole creation with
      // "No item with identifier undefined" (#112, #114).
