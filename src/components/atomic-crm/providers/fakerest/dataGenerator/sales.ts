...
/**
 * Fonctions possibles, pour que la demo reflete la production (NOS-1156).
 *
 * Sans , le selecteur de signataire du contrat reste vide -- il
 * ecarte a dessein les personnes sans fonction, le contrat exigeant nom ET
 * qualite. Un chemin invisible en local est un chemin livre a l aveugle.
 */
const JOB_TITLES = [
...
export const generateSales = (_: Db): Sale[] => {
  const randomSales = Array.from(Array(5).keys()).map((id) => {
    ...
    return {
      ...
      administrator: false,
      disabled: false,
      job_title: random.arrayElement(JOB_TITLES),
    };
  });
  return [
    {
